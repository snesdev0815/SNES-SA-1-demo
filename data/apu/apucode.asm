...
ClearVariables:
	...
	mov SpcReportType,#3		;preset to mod stimulus for demo;reset report type to void
...
;	mov SpcReportInst,#$00	;clear instrument match
	movw	ya,Channel1Array
	movw	CurrentChDataLo,ya	
	mov	CurrentChannel,#$00
...
;there is a new note to trigger, copy data from pattern to current channel data buffer:
	...
	and a,#$1f
	cmp a,SpcReportArg
	bne ModeReportNoInstrumentMatch

	inc SpcReportInst

ModeReportNoInstrumentMatch:
	inc	y
...
;	mov SpecialReportCmdTrigger,#1	;tell report handler to submit stimulus to snes
...
ReportSpecialCmd:
/*
	mov a,SpecialReportCmdTrigger	;check if $e0x or $8xx command was triggered during last row. only report if it was.
	beq	ReportSpecialCmdTriggerDetected
		mov	a,$f5
		cmp a,#$e0
		beq ReportVoid	;only clear ports if snes has acknowledged last report
			ret

	ReportSpecialCmdTriggerDetected:
*/
	mov	$f5,#0			;clear report byte first
	...
	or	a,#$e0
	mov	$f5,a			;return report type 
;	mov SpecialReportCmdTrigger,#0	;clear trigger
	ret
